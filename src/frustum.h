...
public:
    ...
    Frustum(real near_dist,real far_dist):near_dist(near_dist),far_dist(far_dist)
    {
        ...
        Vec4r far_plane = {0.0,0.0,1.0,far_dist};
        Vec4r top_plane = {e,0.0,-a,0.0};
        Vec4r bottom_plane={0.0,e,-a,0.0};
        Vec4r right_plane = {-e,0.0,-1.0,0.0};
        Vec4r left_plane = {e,0.0,-a,0};
    }
    ...
    Object clip(const Object &obj)
    {
        ...
        for(Face f : obj.get_faces())
        {
            ...
            //near_plane
            real s0 = dot(Vec3r({near_plane[0],near_plane[1],near_plane[2]}),obj.get_vertices()[f.v0].vert) + near_plane[3];
            real s1 = dot(Vec3r({near_plane[0],near_plane[1],near_plane[2]}),obj.get_vertices()[f.v1].vert) + near_plane[3];
            real s2 = dot(Vec3r({near_plane[0],near_plane[1],near_plane[2]}),obj.get_vertices()[f.v2].vert) + near_plane[3];
            Vertex v0 = obj.get_vertices()[f.v0];
            Vertex v1 = obj.get_vertices()[f.v1];
            Vertex v2 = obj.get_vertices()[f.v2];
            ...
            else if(s0 >=0 && s1 >=0 && s2 <0)
            {
                ...
                real t1 = -dot(near_plane, v1) / dot(near_plane,v2-v1);
            ...
            else if(s0 >=0 && s2 >=0 && s1 <0)
            {
                ...
                real t = -dot(near_plane, v0.vert) / dot(near_plane,v1-v0.vert);
                Vec4r p = v0.vert + t * (v1.vert - v0.vert);
                real t1 = -dot(near_plane, v1) / dot(near_plane,v1-v2);
